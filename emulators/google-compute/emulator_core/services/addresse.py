...
class Addresse:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.id is not None and self.id != "":
            d["id"] = self.id
        d["kind"] = "compute#addresse"
        d["selfLink"] = f"https://www.googleapis.com/compute/v1/{self.name}"
        return d

class Addresse_Backend:
    def __init__(self):
        self.state = GCPState.get()
        self.resources = self.state.addresses  # alias to shared store

    def _generate_id(self) -> str:
        return str(random.randint(10**17, 10**18 - 1))

    def _generate_name(self, prefix: str = "addresse") -> str:
...
included in the request."""
        ...
        return make_operation(
            ...
            resource_link=f"projects/{project}/regions/{region}/addresses/{resource.name}",
            params=params,
            region=region,
        )
    ...
    def get(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if resource.region and resource.region != region:
            return create_gcp_error(404, f"The resource '{address}' was not found", "NOT_FOUND")
...
`returnPartialSuccess` parameter to `true`."""
        ...
        if not resources:
            ...
            for resource in resources:
                ...
                bucket = items.setdefault(scope_key, {"Addresses": []})
                bucket["Addresses"].append(resource.to_dict())
        return {
            "kind": "compute#addresseAggregatedList",
            "id": f"projects/{project}/aggregated/addresses",
            "items": items,
            "selfLink": "",
        }
...
the specified region."""
        ...
        return {
            "kind": "compute#addresseList",
            "id": f"projects/{project}/regions/{region}/addresses",
...
Resources documentation."""
        ...
        if body is None:
            return create_gcp_error(400, "Required field 'RegionSetLabelsRequest' not specified", "INVALID_ARGUMENT")
    ...
    def move(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        region = params.get("region")
        address = params.get("address")
        body = params.get("RegionAddressesMoveRequest")
        if not project:
            return create_gcp_error(400, "Required field 'project' not specified", "INVALID_ARGUMENT")
        if not region:
            return create_gcp_error(400, "Required field 'region' not specified", "INVALID_ARGUMENT")
        if not address:
            return create_gcp_error(400, "Required field 'address' not specified", "INVALID_ARGUMENT")
        if body is None:
            return create_gcp_error(400, "Required field 'RegionAddressesMoveRequest' not specified", "INVALID_ARGUMENT")
        resource = self._get_resource_or_error(address)
        if is_error_response(resource):
            return resource
        if resource.region and resource.region != region:
            return create_gcp_error(404, f"The resource '{address}' was not found", "NOT_FOUND")
        destination = body.get("destinationAddress") if isinstance(body, dict) else None
        if not destination:
            return create_gcp_error(400, "Required field 'destinationAddress' not specified", "INVALID_ARGUMENT")
        if destination in self.resources:
            return create_gcp_error(409, f"Address '{destination}' already exists", "ALREADY_EXISTS")
...
class addresse_RequestParser:
    ...
    def parse_request(
        ...
        parsers = {
            'testIamPermissions': addresse_RequestParser._parse_testIamPermissions,
            'delete': addresse_RequestParser._parse_delete,
            'aggregatedList': addresse_RequestParser._parse_aggregatedList,
            'get': addresse_RequestParser._parse_get,
            'list': addresse_RequestParser._parse_list,
            'move': addresse_RequestParser._parse_move,
            'setLabels': addresse_RequestParser._parse_setLabels,
            'insert': addresse_RequestParser._parse_insert,
        }
    ...
    def _parse_move(
        ...
        if 'requestId' in query_params:
            params['requestId'] = query_params['requestId']
        # Body params
        params['RegionAddressesMoveRequest'] = body.get('RegionAddressesMoveRequest')
        return params
...
class addresse_ResponseSerializer:
    @staticmethod
    def serialize(
        method_name: str,
        data: Dict[str, Any],
        ...
        serializers = {
            'testIamPermissions': addresse_ResponseSerializer._serialize_testIamPermissions,
            'delete': addresse_ResponseSerializer._serialize_delete,
            'aggregatedList': addresse_ResponseSerializer._serialize_aggregatedList,
            'get': addresse_ResponseSerializer._serialize_get,
            'list': addresse_ResponseSerializer._serialize_list,
            'move': addresse_ResponseSerializer._serialize_move,
            'setLabels': addresse_ResponseSerializer._serialize_setLabels,
            'insert': addresse_ResponseSerializer._serialize_insert,
        }
