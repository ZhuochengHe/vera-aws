...
@dataclass
class GlobalPublicDelegatedPrefixe:
    parent_prefix: str = ""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.id is not None and self.id != "":
            d["id"] = self.id
        d["kind"] = "compute#globalpublicdelegatedprefixe"
        d["selfLink"] = f"https://www.googleapis.com/compute/v1/{self.name}"
        return d

class GlobalPublicDelegatedPrefixe_Backend:
    def __init__(self):
        self.state = GCPState.get()
        self.resources = self.state.global_public_delegated_prefixes  # alias to shared store

    def _generate_id(self) -> str:
        return str(random.randint(10**17, 10**18 - 1))

    def _generate_name(self, prefix: str = "global-public-delegated-prefixe") -> str:
...
parameters that are included in the request."""
        ...
        if name in self.resources:
            return create_gcp_error(
                409,
                f"GlobalPublicDelegatedPrefix {name!r} already exists",
                "ALREADY_EXISTS",
            )
        creation_timestamp = body.get("creationTimestamp")
        if not creation_timestamp:
            creation_timestamp = datetime.now(timezone.utc).isoformat()
        resource = GlobalPublicDelegatedPrefixe(
        ...
        return make_operation(
            ...
            resource_link=(
                f"projects/{project}/global/publicDelegatedPrefixes/{resource.name}"
            ),
            params=params,
        )
    ...
    def get(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        resource = self.resources.get(public_delegated_prefix)
        if not resource:
            return create_gcp_error(
                404,
                f"The resource 'globalPublicDelegatedPrefixes/{public_delegated_prefix}' was not found",
                "NOT_FOUND",
            )
    ...
    def list(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "kind": "compute#globalpublicdelegatedprefixeList",
            "id": f"projects/{project}/global/publicDelegatedPrefixes",
...
patch format and processing rules."""
        ...
        if "name" in body and body.get("name"):
            ...
            if new_name != resource.name and new_name in self.resources:
                return create_gcp_error(
                    409,
                    f"GlobalPublicDelegatedPrefix {new_name!r} already exists",
                    "ALREADY_EXISTS",
                )
...
class global_public_delegated_prefixe_RequestParser:
    @staticmethod
    def parse_request(
        method_name: str,
        ...
        parsers = {
            'list': global_public_delegated_prefixe_RequestParser._parse_list,
            'get': global_public_delegated_prefixe_RequestParser._parse_get,
            'patch': global_public_delegated_prefixe_RequestParser._parse_patch,
            'insert': global_public_delegated_prefixe_RequestParser._parse_insert,
            'delete': global_public_delegated_prefixe_RequestParser._parse_delete,
        }
...
class global_public_delegated_prefixe_ResponseSerializer:
    @staticmethod
    def serialize(
        method_name: str,
        data: Dict[str, Any],
        ...
        serializers = {
            'list': global_public_delegated_prefixe_ResponseSerializer._serialize_list,
            'get': global_public_delegated_prefixe_ResponseSerializer._serialize_get,
            'patch': global_public_delegated_prefixe_ResponseSerializer._serialize_patch,
            'insert': global_public_delegated_prefixe_ResponseSerializer._serialize_insert,
            'delete': global_public_delegated_prefixe_ResponseSerializer._serialize_delete,
        }
