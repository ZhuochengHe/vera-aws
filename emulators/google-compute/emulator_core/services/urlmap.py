...
class UrlMap:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        d["kind"] = "compute#urlmap"
        d["selfLink"] = f"https://www.googleapis.com/compute/v1/{self.name}"
        return d

class UrlMap_Backend:
    def __init__(self):
        self.state = GCPState.get()
        self.resources = self.state.url_maps  # alias to shared store

    def _generate_id(self) -> str:
        return str(random.randint(10**17, 10**18 - 1))

    def _generate_name(self, prefix: str = "url-map") -> str:
        return f"{prefix}-{uuid.uuid4().hex[:8]}"

    def _get_resource_or_error(self, url_map_name: str) -> Any:
    ...
    def _service_exists(self, service_name: str) -> bool:
        ...
        return bool(
            self.state.backend_services.get(name)
            or self.state.backend_buckets.get(name)
            or self.state.backend_services.get(service_name)
            or self.state.backend_buckets.get(service_name)
        )

    def _validate_services(self, body: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        default_service = body.get("defaultService") or ""
        if default_service and not self._service_exists(default_service):
            return create_gcp_error(
                404,
        ...
        for matcher in path_matchers:
            ...
            if matcher_default_service and not self._service_exists(matcher_default_service):
                return create_gcp_error(
                    404,
            ...
            for rule in matcher.get("pathRules") or []:
                ...
                if service and not self._service_exists(service):
                    return create_gcp_error(
                        404,
    ...
    def _filter_resources(self, params: Dict[str, Any]) -> List[UrlMap]:
...
the data included in the request."""
        ...
        error = self._validate_services(body)
        if error:
            return error
        ...
        resource_link = f"projects/{project}/global/urlMaps/{resource.name}"
    ...
    def get(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        resource = self._get_resource_or_error(url_map)
...
`returnPartialSuccess` parameter to `true`."""
        ...
        if not resources:
            ...
            items = {scope_key: {"UrlMaps": [r.to_dict() for r in resources]}}
        return {
            "kind": "compute#urlmapAggregatedList",
            "id": f"projects/{project}/aggregated/urlMaps",
            "items": items,
            "selfLink": "",
        }
...
patch format and processing rules."""
        ...
        if "pathMatchers" in body:
            resource.path_matchers = body.get("pathMatchers") or []
        ...
        if "defaultService" in body:
            resource.default_service = body.get("defaultService") or ""
...
request."""
        ...
        resource.host_rules = body.get("hostRules") or []
        resource.default_url_redirect = body.get("defaultUrlRedirect") or {}
        resource.region = body.get("region") or resource.region
        resource.description = body.get("description") or ""
        resource.default_route_action = body.get("defaultRouteAction") or {}
        resource.fingerprint = body.get("fingerprint") or ""
    ...
    def invalidateCache(self, params: Dict[str, Any]) -> Dict[str, Any]:
        """Initiates a cache invalidation operation, invalidating the specified path,
scoped to the specified UrlMap.

For more information, see [Invalidating cached
content](/cdn/docs/invalidating-cached-con..."""
        project = params.get("project")
        if not project:
            return create_gcp_error(
                400,
        ...
        body = params.get("CacheInvalidationRule") or {}
        if not body:
            return create_gcp_error(
                400,
                "Required field 'CacheInvalidationRule' not found",
                "INVALID_ARGUMENT",
            )
        resource = self._get_resource_or_error(url_map_name)
        if is_error_response(resource):
            return resource

        resource_link = f"projects/{project}/global/urlMaps/{resource.name}"
        return make_operation(
            operation_type="invalidateCache",
            resource_link=resource_link,
            params=params,
        )
...
UrlMap."""
        ...
        body = params.get("UrlMapsValidateRequest") or {}
        if not body:
            return create_gcp_error(
                400,
                "Required field 'UrlMapsValidateRequest' not found",
                "INVALID_ARGUMENT",
            )
        ...
        error = self._validate_services(candidate)
        if error:
            return error
    ...
    def delete(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.resources.pop(url_map_name, None)
        resource_link = f"projects/{project}/global/urlMaps/{url_map_name}"
        return make_operation(
...
class url_map_RequestParser:
    ...
    def parse_request(
        ...
        parsers = {
            'patch': url_map_RequestParser._parse_patch,
            'delete': url_map_RequestParser._parse_delete,
            'insert': url_map_RequestParser._parse_insert,
            'update': url_map_RequestParser._parse_update,
            'invalidateCache': url_map_RequestParser._parse_invalidateCache,
            'aggregatedList': url_map_RequestParser._parse_aggregatedList,
            'testIamPermissions': url_map_RequestParser._parse_testIamPermissions,
            'list': url_map_RequestParser._parse_list,
            'get': url_map_RequestParser._parse_get,
            'validate': url_map_RequestParser._parse_validate,
        }
    ...
    def _parse_invalidateCache(
        ...
        if 'requestId' in query_params:
            params['requestId'] = query_params['requestId']
        # Body params
        params['CacheInvalidationRule'] = body.get('CacheInvalidationRule')
        return params
    ...
    def _parse_validate(
        ...
        params.update(query_params)
        # Body params
        params['UrlMapsValidateRequest'] = body.get('UrlMapsValidateRequest')
        return params


class url_map_ResponseSerializer:
    @staticmethod
    def serialize(
        method_name: str,
        data: Dict[str, Any],
        ...
        serializers = {
            'patch': url_map_ResponseSerializer._serialize_patch,
            'delete': url_map_ResponseSerializer._serialize_delete,
            'insert': url_map_ResponseSerializer._serialize_insert,
            'update': url_map_ResponseSerializer._serialize_update,
            'invalidateCache': url_map_ResponseSerializer._serialize_invalidateCache,
            'aggregatedList': url_map_ResponseSerializer._serialize_aggregatedList,
            'testIamPermissions': url_map_ResponseSerializer._serialize_testIamPermissions,
            'list': url_map_ResponseSerializer._serialize_list,
            'get': url_map_ResponseSerializer._serialize_get,
            'validate': url_map_ResponseSerializer._serialize_validate,
        }
    ...
    @staticmethod
    def _serialize_invalidateCache(data: Dict[str, Any]) -> str:
        return _json.dumps(data)
