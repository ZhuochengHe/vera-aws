...
class ForwardingRule:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.id is not None and self.id != "":
            d["id"] = self.id
        d["kind"] = "compute#forwardingrule"
        d["selfLink"] = f"https://www.googleapis.com/compute/v1/{self.name}"
        return d

class ForwardingRule_Backend:
    def __init__(self):
        self.state = GCPState.get()
        self.resources = self.state.forwarding_rules  # alias to shared store

    def _generate_id(self) -> str:
        return str(random.randint(10**17, 10**18 - 1))

    def _generate_name(self, prefix: str = "forwarding-rule") -> str:
...
the data included in the request."""
        ...
        if name in self.resources:
            return create_gcp_error(409, f"ForwardingRule '{name}' already exists", "ALREADY_EXISTS")
        ...
        resource = ForwardingRule(
            ...
            external_managed_backend_bucket_migration_state=body.get("externalManagedBackendBucketMigrationState", ""),
        ...
        return make_operation(
            ...
            resource_link=f"projects/{project}/regions/{region}/forwardingRules/{resource.name}",
            params=params,
        )
    ...
    def get(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not forwarding_rule:
            return create_gcp_error(400, "Required field 'forwardingRule' not specified", "INVALID_ARGUMENT")
        resource = self.resources.get(forwarding_rule)
        if not resource:
            return create_gcp_error(404, f"The resource '{forwarding_rule}' was not found", "NOT_FOUND")
...
`returnPartialSuccess` parameter to `true`."""
        ...
        if not resources:
            ...
            for resource in resources:
                ...
                bucket = items.setdefault(scope_key, {"ForwardingRules": []})
                bucket["ForwardingRules"].append(resource.to_dict())
        return {
            "kind": "compute#forwardingruleAggregatedList",
            "id": f"projects/{project}/aggregated/forwardingRules",
            "items": items,
            "selfLink": "",
        }
...
project and region."""
        ...
        return {
            ...
            "id": f"projects/{project}/regions/{region}/forwardingRules",
...
same type as the old target."""
    ...
    def delete(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return make_operation(
            ...
            resource_link=f"projects/{project}/regions/{region}/forwardingRules/{forwarding_rule}",
            params=params,
        )


class forwarding_rule_RequestParser:
    @staticmethod
    def parse_request(
        method_name: str,
        ...
        parsers = {
            'insert': forwarding_rule_RequestParser._parse_insert,
            'setLabels': forwarding_rule_RequestParser._parse_setLabels,
            'delete': forwarding_rule_RequestParser._parse_delete,
            'patch': forwarding_rule_RequestParser._parse_patch,
            'aggregatedList': forwarding_rule_RequestParser._parse_aggregatedList,
            'setTarget': forwarding_rule_RequestParser._parse_setTarget,
            'list': forwarding_rule_RequestParser._parse_list,
            'get': forwarding_rule_RequestParser._parse_get,
        }
...
class forwarding_rule_ResponseSerializer:
    @staticmethod
    def serialize(
        method_name: str,
        data: Dict[str, Any],
        ...
        serializers = {
            'insert': forwarding_rule_ResponseSerializer._serialize_insert,
            'setLabels': forwarding_rule_ResponseSerializer._serialize_setLabels,
            'delete': forwarding_rule_ResponseSerializer._serialize_delete,
            'patch': forwarding_rule_ResponseSerializer._serialize_patch,
            'aggregatedList': forwarding_rule_ResponseSerializer._serialize_aggregatedList,
            'setTarget': forwarding_rule_ResponseSerializer._serialize_setTarget,
            'list': forwarding_rule_ResponseSerializer._serialize_list,
            'get': forwarding_rule_ResponseSerializer._serialize_get,
        }
